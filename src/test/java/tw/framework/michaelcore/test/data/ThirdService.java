...
public class ThirdService {
    ...
    public void testSerializable() throws Exception {
        ...
        List<User> users = jdbcTemplate.queryObjectList("SELECT NAME, AGE FROM TT_USER", User.class);
        users.forEach(user -> {
        ...
        List<User> usersb = jdbcTemplate.queryObjectList("SELECT NAME, AGE FROM TT_USER", User.class);
        usersb.forEach(user -> {
            System.out.println(String.format("%s: %s, %s", "A", user.getName(), user.getAge()));
        });
    }
